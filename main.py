...
for i in range(len(files)):
    ...
    barbar = create_barriers(50, 200) # 50, 2117
    ...
    if len(finalfinal) > 1:
        ...
        endendend2 = round(temp_finalfinal[1][0][1]/fps, 3)
    ...
    if not found2:
        ...
        temp_startstartstart_2 = round(finalfinal[1][0][0]/fps, 3)
        temp_endendend_2 = round(temp_finalfinal[1][0][1]/fps, 3)
...
# random.shuffle(video_short_list)

# for vf, st, et in video_short_list:
#     source_path = os.path.join(RUN_DIR, 'run1', vf)
#     if len(video_clips_helper) == 0:
#         end_time_new = st + 0.58
#     elif 1 <= len(video_clips_helper)  <= 2:
#         end_time_new = st + 0.72
#     elif len(video_clips_helper) == 3:
#         end_time_new = st + 0.96
#     else:
#         break
#     print(vf, st, end_time_new)
#     video2 = VideoFileClip(source_path).subclip(st, end_time_new)
#     video_clips_helper.append(video2)
#     print(video_clips_helper)
...
for i1 in range(len(video_clips_short)):
    final_video_clips.append(video_clips_short[i1])
    final_video_4 = final_video_clips[:]
...
for i2 in range(len(video_clips)):
    final_video_clips.append(video_clips[i2])
